...
<%@page import="SignUpPackage.InvoiceViewModel"  %>
<%@page import="java.util.LinkedList" %>
...
<div class="container rounded bg-white mt-5 mb-5" style="margin-left: 25%; padding-top: 2.5%">
    <div class="row">
        ...
        <div style="margin-left: 10%; padding-right: 60%">
            <div class="p-3 py-5">
               ...
               <form name="updatePassword" action="updatePassword.jsp" method="post">
                               <div class="mt-5 text-center"><input name="updatePasswordBtn" class="btn btn-primary profile-button" type="submit" value="Update Password"></div>
               </form>
            </div>
        </div>
                <hr style="margin-left: -20%">
 ...
 <form id="viewOrderHistory" action="RedirectToTransactionPage.jsp" method="get"></form>

<%InvoiceDB idb = new InvoiceDB(); %>
<div class="container" id="order-history">
  <div class="page-header">
    <h1>Order History <small style="font-size: 80%">Total Orders: <%=idb.getNumOfOrders(SignInServlet.userEmail) %></small></h1>
    
    
  </div>

  <div class="flex-col"  style="padding-bottom: 10%">
    <div  style="padding-bottom: 1%">

    <div class="order-history-container" style="padding: 2%">
      <div class="flex-row order-history-header">
        <div class="flex-item order-history-header-item order-num">Order ID</div>
        <div class="flex-item order-history-header-item">Order Date</div>
        <div class="flex-item order-history-header-item num-of-items">No. of Items</div>
        <div class="flex-item order-history-header-item">Total Amount</div>
    ...
    <% LinkedList<InvoiceViewModel> rowsFromDB = idb.getViewRows(SignInServlet.userEmail); %>
    <%for(InvoiceViewModel ivm : rowsFromDB	){ %>
    <div  style="padding-bottom: 1%">
      <div class="flex-row order-row" style="padding: 2%">
        <div class="flex-item order-row-item order-num"><%=ivm.getOrderID()%></div>
        <div class="flex-item order-row-item"><%=ivm.getOrderDate()%></div>
        <div class="flex-item order-row-item num-of-items"><%=ivm.getNumOfItems()%></div>
        <div class="flex-item order-row-item">₹<%=ivm.getTotalAmt()%></div>
		<div ><button type="submit" style="margin-left:40%" name="viewOrderHistoryBtn" form="viewOrderHistory" value="<%=ivm.getOrderID()%>">&nbsp;View</button></div>
      </div>
      </div>
	<%} %>
    </div>
...
<script src="https://use.fontawesome.com/01f444492d.js"></script>
